...
class FPSR_EASEr_model:
    def __init__(self,
        ...
        self.W_sparse = sim.dot_product(self._train.T, self._train, k=self._train.shape[0], format_output='csr')
        diagonal_indices = np.diag_indices(self.W_sparse.shape[0])
        item_popularity = np.ediff1d(self._train.tocsc().indptr)
        self.W_sparse.setdiag(item_popularity + self._l2_norm)

        if torch.cuda.is_available():
            logger.info(f"Use CUDA for Inverse")
            self.W_sparse = torch.tensor(data=self.W_sparse.todense(),
