...
class FPSR_KNN_model:
    def __init__(self,
        ...
        # these are structures needed also for partitioning
        self.update_W()
        ...
        self._asymmetric_alpha = asymmetric_alpha
        self._tversky_alpha = tversky_alpha
        self._tversky_beta = tversky_beta

        if self._similarity == "cosine":
            self.W_sparse = sim.cosine(self._URM.T, k=self._num_neighbors, format_output='csr')
        elif self._similarity == "asym":
            self.W_sparse = sim.asymmetric_cosine(self._URM.T, alpha=self._asymmetric_alpha, k=self._num_neighbors, format_output='csr')
        elif self._similarity == "dot":
            self.W_sparse = sim.dot_product(self._URM.T, k=self._num_neighbors, format_output='csr')
        elif self._similarity == "jaccard":
            self.W_sparse = sim.jaccard(self._URM.T, k=self._num_neighbors, binary=True, format_output='csr')
        elif self._similarity == "dice":
            self.W_sparse = sim.dice(self._URM.T, k=self._num_neighbors, binary=True, format_output='csr')
        elif self._similarity == "tversky":
            self.W_sparse = sim.tversky(self._URM.T, k=self._num_neighbors, alpha=self._tversky_alpha,
    ...
    def predict(self, start, stop):
        ...
        r += self.l_w * torch.sparse.mm(self.W_sparse, user.T).T
        return r
