...
class FPSR_PSGE_model:
    def __init__(self,
        ...
        logger.info(f"Computing global similarity...")
        self._data = data
        self._factors = factors
        self._alpha = alpha
        self._beta = beta
        row, col = self._data.sp_i_train.nonzero()
        self.rate_matrix = csr_matrix((np.ones(len(row), dtype=np.int8), (row, col)),
                                      shape=(self.num_users, self.num_items))
        self.W_sparse = self.global_similarity()
        self.W_sparse = torch.tensor(self.W_sparse, dtype=torch.float).to(self.device)

    def global_similarity(self):
        user_degree = np.array(self.rate_matrix.sum(axis=1))
        d_user_inv = np.power(user_degree, -self._beta).flatten()
        d_user_inv[np.isinf(d_user_inv)] = 0.0
        d_user_inv_diag = diags(d_user_inv)

        item_degree = np.array(self.rate_matrix.sum(axis=0))
        d_item_inv = np.power(item_degree, -self._alpha).flatten()
        d_item_inv[np.isinf(d_item_inv)] = 0.0
        d_item_inv_diag = diags(d_item_inv)

        d_item_alpha = np.power(item_degree, -self._alpha).flatten()
        d_item_alpha[np.isinf(d_item_alpha)] = 0.0
        d_item_alpha = diags(d_item_alpha)

        d_item_inv_alpha = np.power(item_degree, self._alpha).flatten()
        ...
        _, _, vt = svds(int_norm.tocsc(), self._factors)

        return (d_item_alpha @ vt.T) @ (d_item_inv_alpha_diag @ vt.T).T
