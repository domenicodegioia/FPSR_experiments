...
class FPSR_PureSVD_model:
    def __init__(self,
        ...
        _, _, Vt = randomized_svd(self._data.sp_i_train, n_components=self._factors, random_state=self.random_seed)
        self.W_sparse = np.matmul(np.transpose(Vt), Vt)
        self.W_sparse = torch.tensor(self.W_sparse).to(self.device)
